= Tutorial: Configure TLS
:github-directory: https://github.com/hazelcast-guides/hazelcast-platform-operator-tls
:description: Learn how to secure a Hazelcast cluster with TLS using Operator.
...
* Deployed xref:operator:ROOT:get-started.adoc[Operator version 5.8 or above].
* The OpenSSL command line tool, https://www.openssl.org/[openssl].
* The Hazelcast Code Samples repository https://github.com/hazelcast/hazelcast-code-samples cloned to your local machine.

== Create a Kubernetes secret with a TLS certificate and key

. Generate a TLS certificate and key in PEM format using OpenSSL.
+
[source, shell]
----
...
. Create a Kubernetes secret with the TLS certificate and key.
+
[source, shell]
----
...
NOTE: You can also provide a CA certificate to create a truststore. For more information, see xref:operator:ROOT:tls.adoc[TLS].

== Deploy a Hazelcast cluster with TLS
...
. Fetch the external addresses of the cluster:
+
[source, shell]
----
kubectl get hazelcastendpoint --selector="app.kubernetes.io/instance=hazelcast"
NAME            TYPE        ADDRESS
hazelcast       Discovery   198.51.100.18:5701
hazelcast-0     Member      198.51.100.33:5701
hazelcast-1     Member      198.51.100.55:5701
hazelcast-2     Member      198.51.100.37:5701
hazelcast-wan   WAN         198.51.100.18:5710
----

== Deploy Management Center

Enabling TLS for a Hazelcast cluster adds an essential layer of security to protect data during communication. When TLS is enabled, all clients, including Management Center, must be configured with the appropriate TLS certificates to establish a secure connection with the Hazelcast cluster.
...
== Connect to the Hazelcast cluster over TLS
...
----
+
The sample code  for this tutorial is in the link:https://github.com/hazelcast/hazelcast-code-samples/tree/master/platform-operator/hazelcast-platform-operator-tls/docs/modules/ROOT/examples[`docs/modules/ROOT/examples/`] directory.
...
To clean up the created resources, remove all Custom Resources and secrets:
