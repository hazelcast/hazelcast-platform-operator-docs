...
There are two available options for the Expose Externally feature of Hazelcast Platform Operator:
...
Run the following command to create the Hazelcast cluster with the Expose Externally feature enabled using Unisocket type:
...
Run the following command to create the Hazelcast cluster with the Expose Externally feature enabled using Smart type:
...
+
--
NOTE: CLC must be installed in your system — check the installation instructions for CLC: xref:clc:ROOT:install-clc.adoc[].
