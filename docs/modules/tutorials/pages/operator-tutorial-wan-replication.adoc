= Tutorial: Replicate data between two Hazelcast clusters
...
* A deployed xref:operator:ROOT:get-started.adoc[Operator].
...
== Put entries to the maps on the source cluster

In this step, you'll fill the maps on the source cluster.

. Configure the Hazelcast client to connect to the source cluster, using its address. [[configure-client]]
+
...
----
+
The sample code  for this tutorial is in the link:https://github.com/hazelcast/hazelcast-code-samples/tree/master/platform-operator/hazelcast-platform-operator-wan-replication/docs/modules/ROOT/examples/operator-wan[`docs/modules/ROOT/examples/operator-wan`] directory.
