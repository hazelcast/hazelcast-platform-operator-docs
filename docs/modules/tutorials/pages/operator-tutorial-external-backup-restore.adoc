= Tutorial: Restore a cluster from cloud storage
...
* Created blob storage and access credentials in one of the Cloud providers:
- https://aws.amazon.com/s3/[AWS]
- https://cloud.google.com/storage/[GCP]
- https://azure.microsoft.com/en-us/services/storage/blobs/[Azure]
...
. Create a Hazelcast cluster with Persistence enabled:
+
[source, shell]
----
kubectl apply -f - <<EOF
...
NAME               TYPE        ADDRESS
my-hazelcast       Discovery   198.51.100.128:5701
----
+
...
. Create a Map resource with persistence enabled:
+
[source, shell]
----
kubectl apply -f - <<EOF
...
----
+
The sample code for this tutorial is in the link:https://github.com/hazelcast/hazelcast-code-samples/tree/master/platform-operator/hazelcast-platform-operator-external-backup-restore/docs/modules/ROOT/examples/operator-external-backup[`docs/modules/ROOT/examples/operator-external-backup`] directory.
...
Add the cluster configuration to CLC:
...
== Trigger an external backup

To trigger a backup, you need a `bucketURI` where backup data will be stored and a `secret` with credentials for accessing the bucket URI.

. Run one of the following commands to create the secret according to the cloud provider you want to back up:
...
NOTE: If you want to learn more about other authentication methods, see the xref:operator:ROOT:authorization.adoc[Authorization Methods to Access Cloud Storage] documentation.
...
+
List PVCs that were created for the Hazelcast cluster:
+
[source,bash]
----
...
. Create a new Hazelcast cluster.
+
...
Operator starts the agent (as `InitContainer`), which restores the backup data from the external bucket.
+
[source, shell]
----
kubectl apply -f - <<EOF
...
----
+
As you may see, the agent configuration is not set. Operator therefore uses the latest stable version of the agent.
...
----
+
Since we recreated the Hazelcast cluster, services are also recreated. The `ADDRESS` may change.
...
+
Start the client to check the map size and check the restore is successful.
+
[tabs]
====
