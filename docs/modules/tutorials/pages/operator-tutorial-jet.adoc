= Tutorial: Run a data pipeline using Jet
...
TIP: If you want to learn more about other authentication methods, see xref:operator:ROOT:authorization.adoc[Authorization Methods to Access Cloud Storage].
...
NOTE: You need to use Hazelcast 5.3 or above to submit Jet jobs with Operator.

. Check the cluster status to make sure that it's up and running.
...
. Check the output of the Jet pipeline in the logs:
...
. Cancel the Jet job by setting the `spec.state` to `Canceled`. Use one of the following options.

+
.. Cancel the Jet job using `kubectl apply`:
+
[source, shell]
----
kubectl apply -f - <<EOF
...
----
+
.. Cancel the Jet job using `kubectl patch`:
+
[source,bash]
----
...
. Check the Jet job status to make sure the job was cancelled.
...
- Start a Jet pipeline.

- Check a Jet pipeline.

- Cancel a Jet pipeline.
