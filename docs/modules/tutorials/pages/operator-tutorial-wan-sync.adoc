= Tutorial: Synchronize data between two Hazelcast clusters using WAN synchronization
:github-directory: https://github.com/hazelcast-guides/hazelcast-platform-operator-wan-sync
:description: Learn how to synchronize data across two Hazelcast clusters using WAN synchronization.
...
- Synchronize map data between the two Hazelcast clusters using both full and delta WAN synchronization.
...
----
+
The sample code  for this tutorial is in the link:https://github.com/hazelcast/hazelcast-code-samples/tree/master/platform-operator/hazelcast-platform-operator-wan-sync/docs/modules/ROOT/examples/operator-wan-sync[`docs/modules/ROOT/examples/operator-wan-sync`] directory.
...
Add the first cluster configuration to the CLC:
[source, bash]
----
...
for i in {1..10};
do
   clc -c hz-1 map set --name map-sync key-$i value-$i;
done
----
...
[source, bash]
----
clc -c hz-1 map size --name map-sync
----
--
...
+
--
Start the application to populate the map with entries.

[source, bash]
----
cd java
mvn package
java -jar target/*jar-with-dependencies*.jar fill map-sync
----
...
----
Successful connection!
Starting to fill the map (map-sync) with random entries.
Current map size: 2
Current map size: 3
Current map size: 4
....
...
----
NAME                   TYPE        ADDRESS
hazelcast-second       Discovery   198.51.100.16:5701
hazelcast-second-wan   WAN         198.51.100.16:5710
----
+
...
[source, bash]
----
clc -c hz-2 map size --name map-sync
----
--
...
cd clients/java
mvn package
java -jar target/*jar-with-dependencies*.jar size map-sync
----
...
+
If you need a *full WAN synchronization*, run the following command to apply the configuration:
+
[source, shell]
----
kubectl apply -f - <<EOF
...
EOF
----
If you need a *delta WAN synchronization*, run the following to apply the configuration:
+
[source, shell]
----
kubectl apply -f - <<EOF
...
. Create a WAN synchronization resource within the Kubernetes cluster, using the existing `WanReplication` CR.
+
[source, shell]
----
kubectl apply -f - <<EOF
...
[NOTE]
====
WAN synchronization ensures data consistency between the two Hazelcast clusters. Full WAN synchronization transmits all data from the source cluster to the target cluster, aligning the state of the target `IMap` with the source `IMap`. This method is particularly beneficial when the synchronization between two remote clusters is lost due to WAN queue overflows or cluster restarts.
====

. Check the WAN synchronization status:
+
[source, shell]
----
...
- Deploy two Hazelcast clusters.
- Create a Hazelcast map configuration on one of the clusters.
- Synchronize map data between the two Hazelcast clusters using full and delta WAN synchronization.
