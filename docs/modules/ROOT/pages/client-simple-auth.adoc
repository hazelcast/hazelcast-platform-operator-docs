...
[NOTE]
====
`operatorClientSecretName` is mandatory whenever `spec.security` is configured.
Operator uses this Secret to authenticate to the cluster as a client.

For simple authentication, the user in that Secret must also be listed under
`realms.authentication.simple.users` and must have a role that matches
`clientPermissions.all.principal`. Otherwise Operator fails validation.

For JAAS authentication, this field is also required.
See xref:configure-jaas.adoc[Configure JAAS-based authentication].
====
