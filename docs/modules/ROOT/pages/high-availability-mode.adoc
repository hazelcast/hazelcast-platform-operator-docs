...
== High Availability Mode and Subset Routing

Subset routing is a client feature that a client can connect to a subset of members determined by grouping strategy. To benefit from subset routing, high availability mode must be set, because partition groups are determined depending on this parameter. Also, if the subset routing is used on client-side, in the Hazelcast CR, `exposeExternally.type` must be set to `smart`.

Example server and client configurations to run subset routing correctly:

[source,yaml,subs="attributes+"]
----
include::ROOT:example$/hazelcast-high-availability-mode-subset-routing.yaml[]
----
...
SubsetRoutingConfig subsetRoutingConfig = networkConfig.getSubsetRoutingConfig();
