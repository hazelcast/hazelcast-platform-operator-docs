...
:description: The Native Memory Store is a pluggable memory manager. It allows you to set up different configurations, or stores, for your application's data structures on a single JVM. These memory stores form part of a common access layer that scales up to use a configured amount of memory on the JVM.
