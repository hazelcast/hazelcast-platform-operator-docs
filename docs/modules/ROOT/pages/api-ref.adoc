...
#Authors note - is this Operator API docs or custom resource documentation? Which is it?#

== Hazelcast Platform Operator API Docs

Hazelcast Operator simplifies the deployment and management of Hazelcast clusters, and an important element of this is its intuitive API which abstracts the complexity.

For example, consider the task of connecting to Hazelcast running on Kubernetes from outside with a smart client. To do this, you need to create the service per pod manually, making scaling more difficult, or use a third-party tool such as `metacontroller`. However, with the Operator, you can declaratively specify the `exposeExternally` configuration in the Hazelcast CRD, and Operator will take care of creating the service per pod. 

This topic is a reference for the Hazelcast Platform Operator API types.
