...
= Hazelcast Platform Operator API reference

This is a reference for the Hazelcast Platform Operator API types.
These are all the types and fields that are used in Operator CRDs. 

NOTE: This document was generated from comments in the Go code in the `api/`` directory.
...
m| agent | Agent configurations is the configuration for the InitContainer. m| <<AgentConfiguration,AgentConfiguration>> | false | {repository: "docker.io/hazelcast/platform-operator-agent", version: "latest-snapshot"}
