...
- `kind`: The type of custom resource. Either `Map`(default) or `Hazelcast`. Use the `Hazelcast` kind to apply WAN replication to all custom maps created under the given `Hazelcast` resource. Wan Replication can be started for newly created Map resources as well. 
