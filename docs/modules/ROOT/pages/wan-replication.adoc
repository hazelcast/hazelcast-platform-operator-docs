...
- `kind`: The type of custom resource. Either `Map`(default) or `Hazelcast`. Use the  `Hazelcast` kind to apply WAN replication to all custom maps created under the given `Hazelcast` resource.
...
+
NOTE: WanRepliaction can be started for a Map created after the WanReplication resource which refers to it. This means that the creation order of Map and WanReplication resources doesn't matter.
...
You can also monitor the status of WAN replications in xref:get-started.adoc#step-4-start-management-center[Management Center].
