...
|`tls.secretName`
| Name of the TLS secret with target cluster client certificate. If specified WAN replication will encrypt traffic to target cluster with SSL/TLS.
...
The following `WanReplication` resource creates the WAN replication configuration with TLS enabled.

.Example configuration with TLS enabled
[source,yaml,subs="attributes+"]
----
include::ROOT:example$/wan-replication-tls-example.yaml[]
----
