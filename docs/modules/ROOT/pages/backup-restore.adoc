...
==== Optional: Auto Force Start
Additionally, auto force start can be enabled:
...
In this case, the operator will detect whether there are failed Hazelcast members
and the cluster is in *PASSIVE* mode. If that is the case, it will trigger a force start.

WARNING: Force start is a destructive operation. The cluster will lose data.
