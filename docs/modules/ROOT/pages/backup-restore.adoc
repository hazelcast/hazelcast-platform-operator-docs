...
<1> Name of the Hazelcast custom resource used for both backup and restore. If the names do not match, the restore fails.
<2> `HotBackup` resource name used for restore. The backup folder name is taken from the this name.
