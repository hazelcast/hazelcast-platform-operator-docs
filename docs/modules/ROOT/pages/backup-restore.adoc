...
The Operator supports three options for enabling Persistence:

- PVC(Local): Storing data at PVC and restoring the data from it

- PVC(External): Migrating Hazelcast backup data to/from blob storages that cloud providers maintain. PVC is used for temporary storage for the backups.

- HostPath: Storing data at HostPath volume instead of PVC and restoring the data from it

We recommend using PVC, so the examples on this page use this option. If you need to use `HostPath`, see <<hostpath,  HostPath Support for Persistence>>.
...
+
NOTE: Make sure to calculate the total disk space that you will use. The total used disk space may be larger than the size of in-memory data, depending on how many <<single-backup, backups>> you take.
...
=== Backup with External Persistence

For triggering Backup with External Persistence, you need to pass two additional configurations.
One is `bucketURI`. The other one is `secret`. According to chosen cloud provider you need to get
bucket URI and credentials to create secret.
...
. First two steps will be the same with <<Restore-Local, Restoring from Local Backups>>: Deleting the Hazelcast cluster and Checking that the PVCs are still available
...
. If you have not created the secret, you must create the secret using the same way with the <<Create-Secret, Backup with External Persistence>>.
