...
<2> If you configured maps with persistence, you need to create them while creating Hazelcast CR  as well. To be able to create them while creating Hazelcast CR, you need to use xref:custom-config.adoc[Custom Config]. If you try to apply Hazelcast CR and then Map CRs, it will not work.
<3> `pvcNamePrefix` is the prefix of the existing PVCs. It can be `persistence` or `hot-restart-persistence` depending on the installation method (in different versions of Hazelcast Helm Chart and Hazelcast Platform Operators these two values are used by default) of the old cluster. You can decide this value by running `kubect get pvc` command.
...
TIP: To find the `backupFolder` value, you can run a simple pod that lists the contents of the PVC and check its logs, it will print the folder structure of the specified PVC. For example:

[source,yaml,subs="attributes+"]
----
include::ROOT:example$/pod-local-pvc-content.yaml[]
----
<1>  Replace `/data/persistence` with the path where your PV is mounted inside the container.
<2> Replace `/data/persistence` with the correct mountPath specified in PV.
<3> Replace with the name of the one of the PVCs mounted to the cluster that the backup is taken.

NOTE: Agent copies backup to be restored from `{baseDir}/{backupDir}/{backupFolder}` to `/data/persistence/base-dir` path.
