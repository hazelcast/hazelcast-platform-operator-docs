...
=== Scheduling Backups with CronHotBackup Custom Resource

You can schedule regular backups, using the `.spec.schedule` and `.spec.hotBackupTemplate` fields of the `CronHotBackup` resource. You can find more detailed information about the `CronHotBackup` resource in the xref:api-ref.adoc#cronhotbackupspec[API Reference] page.

. Create the `CronHotBackup` resource with `schedule` and `hotBackupTemplate` fields.
+
[source,yaml,subs="attributes+"]
----
...
----
+
`.spec.schedule` field takes a valid cron expression. For example:
+
[cols="1,1"]
|===
| ```30 10 * * *```
...
=== Scheduling External Backups with CronHotBackup Custom Resource

You can schedule external backups, using the `.spec.schedule` and `.spec.hotBackupTemplate` fields of the `CronHotBackup` resource. You can find more detailed information about the `CronHotBackup` resource in the xref:api-ref.adoc#cronhotbackupspec[API Reference] page.
