...
<2> `pvcNamePrefix` is the prefix of the existing PVCs. It can be `persistence` or `hot-restart-persistence` depending on the installation method (in different versions of Hazelcast Helm Chart and Hazelcast Platform Operators these two values are used by default) of the old cluster. You can decide this value by running `kubect get pvc` command.
<3> `baseDir` is the root directory for persistence.
<4> `backupDir` is the directory that contains a backupFolder for each available backup.
<5> `backupFolder` is the directory containing the specific backup for the restore.
