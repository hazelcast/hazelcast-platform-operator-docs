...
NOTE: It is not allowed to override the existing restore data with a backup. This means that if there is a PersistentValue left from the previous Hazelcast cluster, you cannot restore a backup into this PersistentValue.
