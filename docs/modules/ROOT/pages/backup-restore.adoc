...
The Operator supports three options for enabling Persistence: PVC(Local), PVC(External) and `HostPath`. We recommend using PVC, so the examples on this page use this option. If you need to use `HostPath`, see <<hostpath,  HostPath Support for Persistence>>.
...
NOTE: Make sure to use external restore, when you decide to use external backup.
...
The agent configuration is optional. If you give backupType as 'External' and do not pass the agent configuration, the operator
...
. Hazelcast PVCs will be the same with `Local` persistence:
...
=== Hot Backup with Local Persistence
...
=== Hot Backup with External Persistence

For triggering Hot Backup with External Persistence, you need to pass two additional configurations.
One is `bucketURI`. The other one is `secret`. According to chosen cloud provider you need to get
bucket URI and credentials need to create secret.
...
. Create the secret:
+
.. AWS: See https://docs.aws.amazon.com/sdk-for-go/api/aws/session/[AWS Session] to learn about authentication procedure.
+
[source,bash]
----
kubectl create secret generic br-secret-s3 --from-literal=region=<region> \
	--from-literal=access-key-id=<access-key-id> \
	--from-literal=secret-access-key=<secret-access-key>
----

.. GCP: See https://cloud.google.com/docs/authentication/production/[Application Default Credentials] to learn about authentication procedure.
+
[source,bash]
----
kubectl create secret generic br-secret-gcp --from-file=google-credentials-path=<service_account_json_file>
----

.. Azure: See https://docs.microsoft.com/en-us/azure/storage/common/storage-account-keys-manage?tabs=azure-portal/[Azure Storage Account Keys] to learn about authentication procedure.
+
[source,bash]
----
...
== Checking the Status of a Hot Backup

To check the status of a hot backup, run the following command:
[source,bash]
----
kubectl get hotbackup hot-backup
----

The status of the hot backup is displayed in the output.
[source,bash]
----
NAME         STATUS
hot-backup   Success
...
[[Restore-Local]]
=== Restoring from Local Hot Backups

To restore a cluster from local hot backups, you can directly reapply the `Hazelcast` resource, and it will have access to the PVCs that contain the persisted data.
...
=== Restoring from External Hot Backups

To restore a cluster from external hot backups, you must enable external restore by adding `restore configuration` under the persistence.
...
. First two steps will be the same with <<Restore-Local, Restoring from Local Hot Backups>>: Deleting the Hazelcast cluster and Checking that the PVCs are still available
...
. If you have not created the secret, you must create the secret using the same way with the <<Create-Secret, Hot Backup with External Persistence>>.
