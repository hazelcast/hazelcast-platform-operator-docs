...
You can schedule regular backups, using the `.spec.schedule` and `.spec.hotBackupTemplate` fields of the `CronHotBackup` resource
...
[source,bash]
----
kubectl apply -f ./cron-hot-backup.yaml.yaml
----
...
You can schedule external backups, using the `.spec.schedule` and `.spec.hotBackupTemplate` fields of the `CronHotBackup` resource
