...
== Restoring from Persistent Volume Claims

To restore from local backups that are in a existing link:https://kubernetes.io/docs/concepts/storage/persistent-volumes/[Persistent Volume], configure `localConfig`.
...
<1> For existing PVCs to attach successfully to the newly created cluster;

- If the older cluster was created by the Hazelcast Platform Operator, Hazelcast custom resource name for both backup and restore CRs must be the same, otherwise, the restore fails.
- If the older cluster was created by the Hazelcast Helm Chart, the name of the new CR must be in the following format: `<name>-hazelcast-persistence`. For example, assuming older cluster was installed with `helm install hz hazelcast/hazelcast-enterprise` command, new CR name must be `hz-hazelcast-enterprise`.
