...
<3> Optional. This is always `hazelcast/platform-operator-agent`. 
    ...
    Do not configure the agent, Hazelcast Operator uses the latest compatible version of agent.
...
Your Hazelcast pod starts with `platform-operator-agent`, deployed as a sidecar. The agent uploads backups to the external bucket that you have configured. 
...
+
This approach gives the cluster access to the PVCs that contain the persisted data. The Hazelcast cluster is restored from the existing `persistence` folders.
...
+
For this approach, the `platform-operator-agent` container is deployed as an init container
with the Hazelcast container in the same Pod. The agent starts before the Hazelcast container to arrange backup folders in advance of a successful restore.
+
...
Do not pass the agent configuration, Hazelcast Operator uses the latest compatible version of agent.

[restoring-from-external-backups]
== Restore from External Backups

You can use an external backup to persistently restore a backup across clusters.
...
The `platform-operator-agent` container is deployed as an init container with the Hazelcast container in the same Pod. The agent starts before the Hazelcast container 
to download backup folders from the source bucket and arrange them in advance of a successful restore.
