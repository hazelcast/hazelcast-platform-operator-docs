...
|state
|State is used to manage the job state. The default value is 'Running'.

WARNING: The value of `state` field must be `Running` when the JetJob object is created for the first time.
