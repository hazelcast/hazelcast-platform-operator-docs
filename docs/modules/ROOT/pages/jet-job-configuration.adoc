...
The query options in the `bucketURI` can be used for additional configuration. For example, to specify the AWS S3 URL to send requests to, you can use the `endpoint` parameter: `s3://my-bucket?endpoint=my.endpoint.url&disableSSL=true&s3ForcePathStyle=true`.

The following query options are supported:

• region: The AWS region for requests
• endpoint: The endpoint URL (hostname only or fully qualified URI)
• disable_ssl (or disableSSL): A value of “true” disables SSL when sending requests
• s3_force_path_style (or s3ForcePathStyle): A value of “true” forces the request to use path-style addressing
