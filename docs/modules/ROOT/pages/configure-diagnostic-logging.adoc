...
default plugins, see xref:hazelcast:maintain-cluster:monitoring.adoc#diagnostics-plugins[Diagnostics plugins].
...
You can then configure the `com.hazelcast.diagnostics` logger (e.g. a Log4j2 logger and rolling appender) to control size and rollover using time/size policies. For more information, see xref:hazelcast:maintain-cluster:monitoring.adoc#diagnostics-using-logging-frameworks[Diagnostics using logging frameworks].
...
NOTE: System log diagnostics (cluster activity and optional partition migration details) can be enabled and tuned using `hazelcast.diagnostics.systemlog.enabled` and `hazelcast.diagnostics.systemlog.partitions` if needed. For more information, see xref:hazelcast:maintain-cluster:monitoring.adoc#systemlog[SystemLog].
...
For more information about system properties, see xref:hazelcast:maintain-cluster:monitoring.adoc#diagnostics[Enable diagnostics]. 