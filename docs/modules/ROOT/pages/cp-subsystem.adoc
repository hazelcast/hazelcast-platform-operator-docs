...
If `memberCount` is not specified, every cluster member is automatically designated as a CP member. Similarly, if `groupSize` is not specified, all CP members will be included in each CP group by default.
