= Configuring ReplicatedMaps
:description: pass:q[A Replicated Map is a distributed key-value data structure where the data is replicated to all members in the cluster. It provides full replication of entries to all members for high speed access. You can create and configure ReplicatedMaps using the `ReplicatedMap` custom resource.]
