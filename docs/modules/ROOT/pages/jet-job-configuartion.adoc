...
With the Hazelcast Platform Operator, you can run Data Pipelines from existing JAR files for processing data. Data Pipelines, depending on the data source, can be used for stream or batch processing. To understand the Data Pipelines and the Jet Engine, refer to link:https://docs.hazelcast.com/hazelcast/latest/pipelines/overview[Data Pipeline documentation].

== Configuring the Jet Engine

To create a Data Pipeline using `JetJob` CR, the Jet Engine in the `Hazelcast` CR must be configured. It is required to set `enable` and `resourceUploadEnabled` to `true`. 
Below are the configuration options for the Jet Engine in the `Hazelcast` resource. For more detailed information, see the xref:api-ref.adoc#jetengineconfiguration[API Reference].
...
|enabled
|Enables the Jet Engine.

|resourceUploadEnabled
|Enables resource uploading for Jet engine.
