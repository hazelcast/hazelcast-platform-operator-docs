= Module Deployment
:description: Hazelcast Platform Operator allows you to deploy your own custom and domain classes to Hazelcast Plaftorm cluster members. You can deploy your application code from external buckets, ConfigMap objects, and URLs in Kubernetes.
...
Below are the configuration options for the Module Deployment feature. For more information, see the xref:api-ref.adoc#moduledeploymentconfiguration[API Reference].
