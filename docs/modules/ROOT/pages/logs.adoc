...
By default, `kubectl logs <pod>` shows only the first container. To see logs for the helper containers (see xref:deployed-containers.adoc[What Operator deploys]), name them explicitly with `-c`:
