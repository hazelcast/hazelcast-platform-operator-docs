...
To enable TLS when using an LDAP server that employs the LDAPS (LDAP over SSL) protocol or the Start TLS operation, you need to create a secret that contains `keyStore`, `keyStorePassword`, `trustStore` and `trustStorePassword` and ensure that the `tls` parameter is configured to use this secret.
