...
:description: Hazelcast allows you to deploy your own custom and domain classes to cluster members. From Operator, you can deploy your application code from external buckets and ConfigMap objects in Kubernetes.

WARNING: User Code Deployment is deprecated. Use xref:user-code-namespaces.adoc[User Code Namespaces] or xref:module-deployment.adoc[Module Deployment] instead.
...
Operator provides the following configuration options for the User Code Deployment feature. For more information, see the xref:api-ref.adoc#usercodedeploymentconfig[API reference].
...
== Configure an entry processor with operator

An entry processor executes your code on a map entry in an atomic way. To implement the `EntryProcessor` interface, you can deploy an `EntryProcessor` class from Operator to the classpath of a Hazelcast member. See xref:hazelcast:data-structures:entry-processor.adoc[Entry processor] for more information.
