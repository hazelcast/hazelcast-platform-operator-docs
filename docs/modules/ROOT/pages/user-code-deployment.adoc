...
WARNING: Currently, locally mounted drives are NOT supported for `bucketURI`; only cloud provider external buckets are supported.
...
--
====
<1> As warned in the beginning of this section, you cannot provide a `bucketURI` which is locally mounted; you can only provide the mounted cloud directory.
