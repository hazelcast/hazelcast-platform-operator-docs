...
IAM roles for service accounts is a feature that allows you to map a Kubernetes Service Account to a AWS IAM Role so that users can manage pods permissions using IAM. To be able to use it, you need to annotate the service account you created with the following command:
