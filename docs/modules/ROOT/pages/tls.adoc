...
To be able to use `OpenSSL`, you need to add required Jar files into the classpath. You can take advantage of `UserCodeDeployment` feature to install Jars and add them in the classpath.

.Example Hazelcast OpenSSL configuration 
[source,yaml,subs="attributes+"]
----
include::ROOT:example$/hazelcast-tls-openssl.yaml[]
----

<1> Enable UserCodeDeployment to install required Jar files in the classpath. It is assumed that the bucket stores all required jar files. +
    Here is a sample list of Netty Jar dependencies required by BoringSSL implementation:
    - netty-handler-<version>.jar
    - netty-tcnative-boringssl-static-<version>-linux-x86_64.jar
