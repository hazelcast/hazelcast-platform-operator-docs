...
The labels in node selector will match the node's label.
...
nodes with respect to each other but since there might be small of number of nodes, we do not want block
the scheduling if the number of Hazelcast instances are greater than the number of nodes.
...
Let's run the following to taint the node node1:
...
Following configuration cannot schedule any Hazelcast instance into the node `node1`:
...
We need to add tolerations to our Hazelcast pods. It will allow the pods to run on the node `node1`:
