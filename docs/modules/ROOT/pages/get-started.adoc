...
CRDs allow new types of resources tailored to specific use cases to be created, without adding another API server. They also provide detailed reports on the configuration content of a CRD to any authorized Kubernetes API consumer. Using CRDs, you can:
...
For example, the `clusterName` object can be used to rename a cluster. The Operator's controllers watch for the apply event and create resources in the cluster to achieve the changes expressed by the `clusterName` object.
...
#Authors note - Is clean up part of the deployment process or is it only something you'd do if you wanted to remove Operator? In what circumstances would you do this? Perhaps this should be a separate topic and not part of Deployment? Please advise.#
