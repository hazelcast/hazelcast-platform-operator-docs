...
You can deploy Hazelcast Platform Operator by applying the https://repository.hazelcast.com/operator/bundle.yaml[`bundle.yaml`] file. Please apply it to your Kubernetes cluster with the following command.
...
If you choose two deploy multiple Hazelcast Operators in different namespaces, you will need to update the name of every operator. You can add `namePrefix` field in the `kustomization.yaml` to differentiate them.
