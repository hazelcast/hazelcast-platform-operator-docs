...
+
--
Run the following command to *deploy the operator and the CRDs together*. Operator by default watches all namespaces, you can configure the watched namespace with `watchNamespace` variable.
...
+
--
Run the following commands to *deploy the operator and the CRDs separately*. Following operation may be done by an administrator.
...
After installing CRDs, install the operator by running the following command. Following operation requires only namespace scoped permissions for `hz-system` and `watched-namespace` namespaces.
