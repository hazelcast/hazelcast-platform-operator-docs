...
Starting from the Hazelcast Platform Operator 5.6.0, a Helm chart is available to install the operator. It is available from the Hazelcast Helm Charts repository and can be added to your Helm repository list by running the following command:
...
=== Deploy the Hazelcast Platform Operator and the CRDs together
...
=== Deploy the Hazelcast Platform Operator and the CRDs separately

This avoids installing any cluster-scoped resources
...
After installing CRDs, the operator can be installed by running the following command:
...
If you install the CRDs separately from the operator, you need to remove the CRDs by running the following command:
